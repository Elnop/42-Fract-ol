...
/*   Updated: 2023/01/26 04:05:19 by lperroti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	novabrot_px_iter(t_complex z, size_t max_iter)
{
	size_t		iter;
	double		abs_z;
	t_complex	c;
...
void	put_novabrot(t_mlxapp app)
{
	...
	while (y < WINDOW_WIDTH)
	{
		...
		{
			last_iter = novabrot_px_iter(pos_to_complex(app, x, y),
					app.max_iter);
			if (last_iter)
